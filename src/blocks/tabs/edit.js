				...
				<div className={ className }>
					<div className={ tabListClasses } role="tablist">
						{ tabsData.map( ( tabData, index ) =>
							<button key={ index } className={ `${ defaultClassName }__tab${ currentTab === index + 1 ? ' is-active' : '' }` }
								onClick={ () => {
							...
							>
								<Tooltip text={ __( 'Sort tab', 'fleximpleblocks' ) }>
									<span className={ `${ defaultClassName }__icon ${ defaultClassName }__icon--drag` } role="button">
