	...
	if ( isset( $attributes['displayAudio'] ) && $attributes['displayAudio'] ) {
		...
		if ( ! empty( $audios ) ) {
			...
			foreach( $audios as $audio ) {
				if ( $i > 0 ) { break; } // Display only the first audio item
				$post_audio = sprintf(
	...
	foreach ( $attributes['orderArticle'] as $article_fragment ) {
		if ( 'media' === $article_fragment && isset( $attributes['displayMedia'] ) && $attributes['displayMedia'] ) {
			...
			foreach ( $attributes['orderMedia'] as $media_fragment ) {
				...
				if ( 'audio' === $media_fragment && $post_audio ) { $post_markup .= $post_audio; }
			}
			$post_markup .= '</div>';
		}
		...
		if ( 'content' === $article_fragment && isset( $attributes['displayContent'] ) && $attributes['displayContent'] ) {
			...
			foreach ( $attributes['orderContent'] as $content_fragment ) {
				if ( 'categories' === $content_fragment && $post_categories ) { $post_markup .= $post_categories; }
				if ( 'title' === $content_fragment && $post_title ) { $post_markup .= $post_title; }
				if ( 'meta' === $content_fragment && isset( $attributes['displayMeta'] ) && $attributes['displayMeta'] ) {
					...
					foreach ( $attributes['orderMeta'] as $meta_fragment ) {
						if ( 'author' === $meta_fragment && $post_author ) { $post_markup .= $post_author; }
						if ( 'date' === $meta_fragment && $post_date ) { $post_markup .= $post_date; }
						if ( 'comments' === $meta_fragment && $post_comments ) { $post_markup .= $post_comments; }
					}
					$post_markup .= '</div>';
				}
				if ( 'excerpt' === $content_fragment && $post_excerpt ) { $post_markup .= $post_excerpt; }
				if ( 'readMore' === $content_fragment && $post_read_more ) { $post_markup .= $post_read_more; }
			}
			$post_markup .= '</div>';
		}
