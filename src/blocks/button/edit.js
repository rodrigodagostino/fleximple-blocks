...
function ButtonEdit( {
	...
	const buttonStyles = {
		...
		borderRadius: borderRadius.value ? borderRadius.value + borderRadius.unit : undefined,
	}

	const relAttribute = `${ noFollow ? 'nofollow' : '' } ${ noReferrer ? 'noreferrer' : '' }`.trim()

	const iconClasses = classnames(
		`${ defaultClassName }__icon`, {
			[ iconId ]: iconId,
	...
	const customIconClasses = classnames(
		`${ defaultClassName }__custom-icon`, {
			[ `position-${ iconPosition }` ]: iconPosition && !isIconOnly,
		},
	)
	...
	return (
		<>
			...
			<InspectorControls>
				<PanelBody title={ __( 'Main', 'fleximpleblocks' ) }>
					...
					<TextControl
						...
						help={ __( 'It will show to the user as a tooltip.', 'fleximpleblocks' ) }
					/>
				...
				<PanelBody title={ __( 'Icon', 'fleximpleblocks' ) } initialOpen={ false }>
					...
					{ !isIconOnly &&
						<BaseControl>
							<p className="fleximple-components-control__label">{ __( 'Icon position', 'fleximpleblocks' ) }</p>
					...
					{ !!hasCustomIcon &&
						<TextareaControl
							...
							help={ __( 'Insert the HTML code for your custom icon.', 'fleximpleblocks' ) }
							value={ customIcon }
				...
				<SpacingPanel
					...
					controls={ [ 'padding-top', 'padding-left', 'padding-right', 'padding-bottom' ] }
					initialOpen={ false }
			...
			<div { ...blockProps }>
				<div className={ buttonClasses } style={ buttonStyles } rel={ relAttribute ? relAttribute : null }>
					...
					{ !!hasCustomIcon && ( iconPosition === 'right' || iconPosition === undefined ) &&
					<RawHTML className={ customIconClasses } style={ { height: iconSize } }>
						{ customIcon }
					</RawHTML>
					}

					{ !!iconId && ( iconPosition === 'right' || iconPosition === undefined ) && !hasCustomIcon &&
					<span><i className={ iconClasses } style={ { fontSize: iconSize } } /></span>
					}
				</div>
			</div>
		</>
