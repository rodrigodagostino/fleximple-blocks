		...
		foreach ( $attributes['orderArticle'] as $article_fragment ) {
			...
			if ( 'content' === $article_fragment && isset( $attributes['displayContent'] ) && $attributes['displayContent'] ) {
				...
				foreach ( $attributes['orderContent'] as $content_fragment ) {
					if ( 'categories' === $content_fragment && $post_categories ) { $post_carousel_markup .= $post_categories; }
					if ( 'title' === $content_fragment && $post_title ) { $post_carousel_markup .= $post_title; }
					if ( 'meta' === $content_fragment && isset( $attributes['displayMeta'] ) && $attributes['displayMeta'] ) {
						...
						foreach ( $attributes['orderMeta'] as $meta_fragment ) {
							if ( 'author' === $meta_fragment && $post_author ) { $post_carousel_markup .= $post_author; }
							if ( 'date' === $meta_fragment && $post_date ) { $post_carousel_markup .= $post_date; }
							if ( 'comments' === $meta_fragment && $post_comments ) { $post_carousel_markup .= $post_comments; }
						}
						$post_carousel_markup .= '</div>';
					}
					if ( 'excerpt' === $content_fragment && $post_excerpt) { $post_carousel_markup .= $post_excerpt; }
					if ( 'readMore' === $content_fragment && $post_read_more ) { $post_carousel_markup .= $post_read_more; }
				}
