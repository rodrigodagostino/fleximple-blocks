...
import { BaseControl, Button, PanelBody, Placeholder, SelectControl, TextControl } from '@wordpress/components'
...
function AdEdit( {
	...
	if ( !! mediaData ) {
		...
		Object.entries( id ).reverse().forEach( ( [ key, value ], index, array ) => {
			if ( value ) {
				pictureSources.push(
					<source
			...
			<picture { ...blockProps }>
				{ ! id.small && ! id.medium && ! id.large && (
					<Placeholder
						...
						instructions={ __( 'Select an image to start with.', 'fleximpleblocks' ) }
					>
						<MediaUpload
							id={ `fleximple-blocks-container-media-control-${ instanceId }` }
							onSelect={ ( media ) => {
